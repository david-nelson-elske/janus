/**
 * AdrDetail — Detail view for a single ADR entity.
 */
...
export interface AdrDetailProps {
  readonly context: BindingContext;
  readonly config: BindingConfig;
}

export function AdrDetail({ context, config }: AdrDetailProps) {
  if (!context) return h('div', null, 'No ADR found');
